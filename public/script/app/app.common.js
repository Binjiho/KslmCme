...
const ajaxSuccessData = (obj) => {
    ...
    if (obj.popOpen) {
        // console.log(obj.popOpen);
        // return false;

        const url = obj.popOpen.popOpen.url;
        const popName = isEmpty(obj.popOpen.popOpen.name) ? 'popup' : obj.popOpen.popOpen.name;
        const popupHeight = isEmpty(obj.popOpen.popOpen.height) ? 700 : obj.popOpen.popOpen.height;
        const popupWidth = isEmpty(obj.popOpen.popOpen.width) ? 500 : obj.popOpen.popOpen.width;
        const popupY = (window.screen.height / 2) - (popupHeight / 2);
        const popupX = (window.screen.width / 2) - (popupWidth / 2);
